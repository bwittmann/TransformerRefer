...
class RefNetV2(nn.Module):
    def __init__(self,
        ...
        if not no_reference:
            # language encoding: descriptions -> vectors (including attention and language classification, etc...)
            self.lang = LangModule(num_class, use_lang_classifier, use_bidir, emb_size, hidden_size)

            # proposal matching: match the generated proposals and select the most confident one
